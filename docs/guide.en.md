...
`time.inverter_deye_program_6_time`). For another device name, print a package with your prefix:
...
Generated by `tools/dashboards/build.py` with placeholder entity IDs: a controller status section
(`deye_controller.yaml`, native cards), a load / grid / PV / SoC history-and-plan chart with plan-state
bands (`plan_chart.yaml`) and a Consumer Compass level timeline (`consumer_compass_chart.yaml`), both for
ApexCharts Card. See the [installation guide](installation.md#dashboard-examples).
