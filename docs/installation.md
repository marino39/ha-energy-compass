...
The panel's SOC forecast and the two charts need [ApexCharts Card](https://github.com/RomRider/apexcharts-card) 2.2.3 or later. The easiest way to get a section with your own entities, capacity and language is the [YAML builder](https://marino39.github.io/ha-energy-compass/builder.html) ([`builder.html`](builder.html) in a checkout): pick a section, fill in the entity IDs and copy the result. The files are generated by `tools/dashboards/build.py`; from a checkout you can also print a section with your entities, capacity and language, for example `python tools/dashboards/build.py --print plan --lang pl --capacity 25 --entity plan=sensor.my_plan --entity valid=binary_sensor.my_forecast_valid --entity soc=sensor.my_battery_soc --entity load=sensor.my_load_power --entity grid=sensor.my_grid_power --entity pv=sensor.my_pv_power`.
...
Copy [the blueprint](../blueprints/automation/energy_compass/notifications.yaml) into Home Assistant's `config/blueprints/automation/energy_compass/notifications.yaml`, then go to **Settings → Automations & scenes → Blueprints → Create automation**. Select **Energy Compass window notifications**. In the blueprint form, select the consumption compass, plan, forecast-valid, Alert, optimizer status and three window entities from the same integration instance, and the **Language** of the messages (English or Polish). By default, the automation uses the current resolved notification preferences from that plan, including later integration-option changes. Turn on **Override integration notification settings** only when this automation needs its own events, timing, quiet hours, cooldown, or cap; the selectors below that toggle then take precedence. **Notify enabled** in the integration still controls both modes. If plan preferences are missing or incomplete, the automation waits for a valid plan instead of sending.

Choose at least one **Notification action** in the blueprint's action selector; that sequence is the only action the automation runs. The variables `notification_title`, `notification_message` and `event_kind` (`favorable` or `limit`) are available to it, for example a `notify.mobile_app_…` action with title `{{ notification_title }}` and message `{{ notification_message }}`. An empty action sequence sends nothing and does not use a window identity or daily allowance. The integration form offers the two events this version handles: **favorable** and **LIMIT**. Existing saved action or other event preferences remain readable but are not dispatched by this blueprint.

Before creating the automation, use **Settings → Devices & services → Helpers → Create helper** to create two **Text** helpers for the last favorable and LIMIT windows (maximum length 255), one **Number** helper for the daily count (minimum 0, maximum at least your daily cap, step 1), one **Date and/or time** helper with **date only** for the last local date, and one **Date and/or time** helper with **both date and time** for the last send. Select all five in the blueprint form. Each text helper stores the last window as JSON with its start and end Unix times, for example `{"s": 1790341200, "e": 1790348400}`. Leave their initial values empty so Home Assistant restores their last values after restart. Do not share the helpers between multiple automations.

The blueprint examines the plan's current favorable windows and upcoming LIMIT windows on forecast, Alert and optimizer changes and each minute. It sends nothing unless the forecast is valid, the Alert is off, the optimizer is `ready` and the plan is still valid; these gates are checked again after the helpers are written, just before the action runs. A favorable window qualifies when it has started and at least the selected time remains; LIMIT fires within the selected lead time, including while it is already running. Windows that overlap the stored one are the same window: recalculations and BOOST/CHEAP changes extend the stored span instead of notifying again, and a new notification needs a window that does not overlap. When both qualify, LIMIT wins. The title says **Use the cheap energy** only when the whole rest of the window is BOOST. Empty first-use text/date/time values are accepted while the count is zero. An unknown count, an unknown date/time after the first action, or any unavailable helper suppresses actions until the helpers recover. During testing, choose an **Event** action with a local event name and inspect the automation trace; do not choose a real notify action until the behavior suits your household.
