...
Choose at least one **Notification action** in the blueprint's action selector. The integration's `actions_configured` plan attribute is informational and any actions entered in integration options are not executed by this blueprint; only its selected action sequence runs. An empty action sequence sends nothing and does not use a window identity or daily allowance. This version of the window blueprint handles **favorable** and **LIMIT** events; the integration's `invalid` event choice is not dispatched by it.
