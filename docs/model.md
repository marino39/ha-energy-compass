...
`minimum_replan_seconds` (Performance, default 120, 0 disables) is the shortest pause between a published result and the next calculation started by an input change. Each solve may use its whole time budget, so without the pause inputs that change faster than a solve kept the optimizer permanently `calculating` and `ready` lasted milliseconds. During the pause the published plan stays `ready`; changes are collected and solved once when it ends. Periodic refreshes, configuration and registry changes and invalid inputs are not delayed.
