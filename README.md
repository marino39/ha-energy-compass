...
In HACS, open **Custom repositories**, add `https://github.com/marino39/ha-energy-compass` as an **Integration**, then download Energy Compass. Restart Home Assistant, and add **Energy Compass** under **Settings → Devices & services → Add integration**. For a manual installation, copy `custom_components/energy_compass` into the Home Assistant config directory's `custom_components` folder and restart. The optional notification blueprint lives outside the HACS-installed integration folder and must be [copied separately](docs/installation.md#opt-in-notifications). The same holds for the optional [Deye (Solarman) controller](docs/installation.md#deye-inverter-controller) — a blueprint plus a package that execute the plan on a Deye inverter — and the [dashboard examples](docs/installation.md#dashboard-examples).
